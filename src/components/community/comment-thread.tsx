...
export function CommentThread({ post, comments }: { post: CommunityPost; comments: CommunityComment[] }) {
  return (
    <section className="mk-panel mk-comments">
      ...
      <ReplyForm postId={post.id} slug={post.slug} />
      <div className="mk-stack">
        {comments.length === 0 ? <p className="text-muted">No responses yet. Add the first field observation.</p> : comments.map((comment) => <CommentItem comment={comment} post={post} key={comment.id} />)}
      </div>
    </section>
  );
}

function CommentItem({ comment, post }: { comment: CommunityComment; post: CommunityPost }) {
  return (
    <article className={`mk-comment ${comment.isAccepted ? 'is-accepted' : ''} ${comment.isModeratorNote ? 'is-mod-note' : ''}`}>
      ...
      <ReplyForm postId={post.id} slug={post.slug} parentId={comment.id} compact />
      {comment.replies.length > 0 && (
        <div className="mk-replies">
          {comment.replies.map((reply) => (
            <CommentItem comment={reply} post={post} key={reply.id} />
          ))}
        </div>
      )}
    </article>
...
function ReplyForm({ postId, slug, parentId, compact = false }: { postId: string; slug: string; parentId?: string; compact?: boolean }) {
  return (
